...
// ─── Sub-components ───────────────────────────────────────
const TypeTab = ({ value, label, accentColor, activeValue, onSelect }) => (
    <button
        onClick={() => onSelect(value)}
        className="relative px-5 py-2 text-[10px] tracking-[0.2em] uppercase transition-all duration-200 font-mono"
        style={{
            backgroundColor: activeValue === value ? (accentColor || '#000') : 'transparent',
            color: activeValue === value ? '#fff' : '#000',
            border: `1px solid ${activeValue === value ? (accentColor || '#000') : 'rgba(0,0,0,0.12)'}`,
        }}
    >
        {label}
        {activeValue === value && (
...
const RangeChip = ({ value, label, activeValue, onSelect }) => (
    <button
        onClick={() => onSelect(value)}
        className="px-4 py-1.5 text-[9px] tracking-[0.18em] uppercase transition-all duration-200 font-mono"
        style={{
            backgroundColor: activeValue === value ? '#000' : 'transparent',
            color: activeValue === value ? '#fff' : 'rgba(0,0,0,0.45)',
            border: `1px solid ${activeValue === value ? '#000' : 'rgba(0,0,0,0.08)'}`,
        }}
    >
        {label}
    </button>
...
export default function RecentTransactions({ transactions, onRefresh }) {
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return filtered;
    }, [transactions, viewMonth, viewYear, timeRange, filterType, searchTerm, sortOrder, isCurrentMonth, now]);
    ...
    return (
        ...
        >
            ...
            <div className="flex gap-2 mb-4">
                <TypeTab value="all" label="All" accentColor="#000" activeValue={filterType} onSelect={setFilterType} />
                <TypeTab value="expense" label="Expenses" accentColor="#b91c1c" activeValue={filterType} onSelect={setFilterType} />
                <TypeTab value="income" label="Income" accentColor="#15803d" activeValue={filterType} onSelect={setFilterType} />
            </div>

            {/* ─── Time Range Toggle ────────────────────────── */}
            <div className="flex gap-2 mb-5">
                {isCurrentMonth && (
                    <RangeChip value="week" label="This Week" activeValue={timeRange} onSelect={setTimeRange} />
                )}
                <RangeChip value="month" label={isCurrentMonth ? 'Full Month' : `All of ${MONTH_NAMES[viewMonth]}`} activeValue={timeRange} onSelect={setTimeRange} />
            </div>
